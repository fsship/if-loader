...
module.exports = function(source, collects) {
  ...
  var define = options['if-loader'];

  console.log('loader-define:', define)

  for (var i = collects.length - 1; i >= 0; i--) {
    var collect = collects[i];
    var predicate = collect.if.predicate;

    var defs = collect.if.value.replace(/(#if-loader |#if |#ifnot )/ig, '').trim()

    if (defs.indexOf(',') > -1) defs = defs.split(',')
    else defs = [defs]

    var exist = defs.indexOf(define) !== -1;
